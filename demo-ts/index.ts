...
document.getElementById("file")!.onchange = function (event: any) {
	CSVFileValidator<CSVRow>(event.target.files[0], CSVConfig).then(
		(csvData: ParsedResults) => {
			csvData.inValidMessages.forEach((message: RowError) => {
				document
					.getElementById("invalidMessages")
					?.insertAdjacentHTML("beforeend", message.message);
			});
