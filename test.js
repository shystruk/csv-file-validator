...
const uniqueError = (headerName, rowNumber) =>
	`<div class="red">${headerName} is not unique at the <strong>${rowNumber} row</strong></div>`;
...
const CSVConfig = {
	headers: [
		...
		},
		{
			...
			inputName: "country",
			optional: true,
			dependentValidate: isRoleForCountryValid,
		},
	],
};
...
const CSVInvalidFileTooManyFields = ['First Name;', 'Vasyl;Stokolosa;'].join(
	'\n'
);
...
test('should return the message "config headers are required"', async (t) => {
	...
	t.deepEqual(csvData.inValidMessages, [
		{
			message: "config headers are required",
		},
	]);
...
test("file without headers, the file is valid and headers are optional", async (t) => {
	const csvData = await CSVFileValidator(CSVValidFileWithoutHeaders, {
		...CSVConfig,
...
test("file with headers, the file is valid and headers are optional", async (t) => {
	const csvData = await CSVFileValidator(CSVValidFile, {
		...CSVConfig,
		isHeaderNameOptional: true,
	});
...
test("fields are mismatch: too many fields", async (t) => {
	const csvData = await CSVFileValidator(CSVInvalidFileTooManyFields, {
		headers: [CSVConfig.headers[0]],
	});

	t.is(csvData.inValidMessages.length, 1);
	t.is(
		csvData.inValidMessages[0].message,
		"Number of fields mismatch: expected 1 fields but parsed 3. In the row 1"
	);
	t.is(csvData.data.length, 1);
});

test("fields are mismatch: not enough fields", async (t) => {
	const csvData = await CSVFileValidator(CSVInvalidFileNotEnoughFields, {
		headers: [
			CSVConfig.headers[5],
			CSVConfig.headers[0],
			CSVConfig.headers[1],
	...
	t.is(
		csvData.inValidMessages[0].message,
		"Number of fields mismatch: expected 3 fields but parsed 2. In the row 1"
	);
	t.is(csvData.data.length, 2);
});
