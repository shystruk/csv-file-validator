...
const headers = CSVConfig.headers.map(i => i.name).join(';');
...
test('should validate .csv file without headers and return data, file is valid and headers are optional', async t => {
    const csvData = await CSVFileValidator(CSVValidFileWithoutHeaders, { ...CSVConfig, nameOfTheRowOptional: true });
...
test('should validate .csv file with headers and return data, file is valid and headers are optional', async t => {
    const csvData = await CSVFileValidator(CSVValidFile, { ...CSVConfig, nameOfTheRowOptional: true });
